...
Selamat pagi semua, terima kasih sudah hadir. Sebelum saya tunjukkan kode apa pun, coba bayangin gini dulu. Ada tiga analis duduk di satu ruang trading. Yang pertama fokus pelototin angka — moving average, momentum, semua indikator di layar. Yang kedua baca chart kayak baca radiograph, nyari bentuk dan pola. Yang ketiga pantau *order flow* — siapa yang lagi beli, siapa yang lagi jual, *real-time*. *Candle* Bitcoin lima belas menit yang sama, tiga pasang mata yang sama sekali beda. Tiap analis nulis catatan singkat, geser ke meja bosnya, terus bos pakai rumus tetap buat mutusin: beli, jual, atau diem dulu. **Itu LLM bot kita.** Dalam tiga puluh menit ke depan, saya bakal tunjukin gimana ruang trading itu kerja — dari ujung ke ujung, dari byte yang datang dari exchange sampai *trade* yang ter-*fill*.
...
[NOTE TO PRESENTER] Bangun *tone*: ini talk *data engineering* yang kebetulan tentang *trading bot*, bukan talk *trading* yang kebetulan pakai data. Analogi "tiga analis" jadi benang merah — pakai ulang kapan pun materi teknis mulai berat.

[SLIDE 2: Pertanyaan Penelitian]
[00:50]

Pertanyaan penelitiannya lugas. Apakah ruang trading tiga-analis ini — yang baca data pasar mentah yang sama persis dengan *rulebook* deterministik — bisa kasih keputusan *trading* yang kompetitif? Dan saya mau jujur dari menit pertama: jawaban yang kami ukur bukan jawaban yang kebanyakan orang harapkan. Bot LLM kami **rugi** di *window* uji. Itu temuannya. Dan justru itu yang bikin studi ini bisa dipertanggungjawabkan secara akademis — kami bangun *comparison harness* yang *byte-deterministic* dan *reproducible*, terus kami laporin apa yang keluar. Bukan hasil yang kami harapkan, tapi hasil yang kami ukur.
...
[NOTE TO PRESENTER] Beri jeda dua detik setelah "rugi" — biarkan tertanam. Lanjutkan dengan percaya diri.
...
Oke, jadi sebelum ketiga analis kita bisa ngapa-ngapain, mereka butuh data. Dan data itu harus dari sumber yang nyata. Kami ambil data pasar dari **Binance Spot**, exchange crypto terbesar berdasarkan volume. Aset-nya BTC terhadap USDT, *timeframe* lima belas menit per *bar*, *window* uji lima hari — sepuluh sampai lima belas April 2025. Totalnya empat ratus delapan puluh *bar*. Bayangin tiap *bar* itu kayak satu "snapshot" pasar, satu per lima belas menit — itulah bahan baku yang bakal "dikunyah" sama tiga analis kita.

Kami panggil dua endpoint REST. Pertama, `GET /api/v3/klines`, yang ngasih data *candlestick* plus satu field tambahan yang krusial — `taker_buy_volume`. Sebentar lagi saya cerita kenapa field itu penting. Kedua, `GET /api/v3/aggTrades`, yang ngasih tiap tick *trade*. Tapi sebenernya endpoint aggTrades udah nggak kami pakai di *hot path* — kami nemu *speedup* enam puluh sampai dua ratus kali dengan cukup pakai kline saja. Kode *fetching*-nya ada di `data/downloader.py` baris 42, lewat library ccxt method `publicGetKlines`.
...
Nah, inilah tampilan respons API mentahnya, byte per byte. Binance ngasih tiap *candle* sebagai array JSON dua belas elemen. Elemen ke-nol itu *open timestamp* dalam milidetik. Terus empat string harga — *open*, *high*, *low*, *close*. Terus volume, *close timestamp*, *quote-asset volume*, jumlah *trade*, dan field yang paling penting buat kita: `taker_buy_base_volume`. Plus dua field lagi yang kami abaikan.

Ada dua hal yang sering bikin orang ketipu di sini. Pertama, semua harga dan volume datang sebagai **JSON string**, bukan angka. Jadi kami *cast* sendiri ke float di `downloader.py` baris 113. Kedua, dari dua belas field tadi kami cuma simpan tujuh: timestamp, OHLCV, dan `taker_buy_volume`. Sisanya kami buang langsung saat *write*. Kami sengaja selektif soal apa yang masuk ke *pipeline*.
...
Jadi kenapa kami sebegitu peduli sama satu field itu? Karena field itulah yang ngasih analis ketiga kita — si ahli *order flow* — apa yang dia butuhin buat kerja, **tanpa harus bayar data tick yang mahal**. Metriknya namanya *cumulative volume delta*, atau CVD. Bayangin kayak papan skor yang berjalan: tiap kali pembeli agresif "narik" *offer*, skor naik; tiap kali penjual agresif "ngentak" *bid*, skor turun. Dari waktu ke waktu, kelihatan deh siapa yang menang — pembeli atau penjual.

Matematikanya cuma identitas dua baris. *Volume* sama dengan taker-buy plus taker-sell. *CVD-delta* sama dengan taker-buy minus taker-sell. Kurangi yang satu dari yang lain, dapat: `cvd_delta sama dengan dua kali taker_buy_volume minus volume`. Selesai. Kami dapat CVD gratis dari data yang udah ada. Kodenya: `data/cvd.py` baris 50, fungsi `cvd_from_klines`. Perubahan ini doang ngasih kami *speedup* enam puluh sampai dua ratus kali dibanding ngunduh aggTrades per *bar*. Jadi analis *order flow* kita sekarang udah punya papan skornya, murah.
...
Setelah *preprocessing*, *engine* mengonsumsi *stream* dataclass `Bar` Python. Bayangin tiap `Bar` itu kayak satu baris rapi di buku log ruang trading kita — semua yang analis butuh tahu tentang satu irisan pasar lima belas menit. Sembilan field total: *timestamp*, OHLC, *volume*, *taker_buy_volume*, CVD kumulatif, dan CVD delta *bar* ini. Satu `Bar` per interval lima belas menit. Total 480 objek `Bar`.

*Loader* di `data/loader.py` baris 19 menegakkan keselarasan *timestamp* secara ketat antara OHLCV CSV dan CVD parquet — kalau ada satu *timestamp* aja yang nggak sinkron, dia langsung *raise* `ValueError` dan *run* berhenti total. Kami nggak pernah ngebiarin data *silently misalign*. *Bug* macam itu — *bar* yang beda pura-pura jadi *bar* yang sama — bakal ngeracunin semua hilir-nya. Mendingan *crash* keras daripada korupsi diam-diam.
